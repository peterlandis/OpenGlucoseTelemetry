...
3. **Future timestamp policy** — Either document that Swift omits **15-minute skew** (`FUTURE_SKEW_MS` in `runtimes/typescript/collectors/validation/semantic.ts`) or add equivalent policy to Swift and note it in the matrix.
...
| D4 | TS code or tests only if matrix requires behavior change | `runtimes/typescript/collectors/normalization/normalize.ts`, `runtimes/typescript/collectors/validation/semantic.ts`, tests |
